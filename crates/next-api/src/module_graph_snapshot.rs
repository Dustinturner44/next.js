...
pub async fn get_module_graph_snapshot(
    ...
    let module_graph_ref = module_graph.read_graphs().await?;
    ...
    let entry_modules_vec: Vec<ResolvedVc<Box<dyn Module>>> = if let Some(entry_modules) = entry_modules {
        entry_modules.await?.iter().copied().collect()
    } else {
        let entries: Vec<_> = module_graph_ref.entries().collect();
        entries.iter().flat_map(|e| e.entries()).collect()
    };

    module_graph_ref
        .traverse_edges_from_entries_bfs(entry_modules_vec.iter().copied(), |parent_info, module| {
            let module_index = get_or_create_module(&mut modules, &mut module_to_index, module);

            if let Some((parent_module, ref_data)) = parent_info {
                let parent_index =
