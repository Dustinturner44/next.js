...
impl Transition for NextCssClientReferenceTransition {
    ...
    async fn process(
        ...
        let new_reference_type = match reference_type {
            ReferenceType::Css(CssReferenceSubType::ModuleStyles) => {
                // Prevent an infinite loop since this transition matches on the
                // InternalModuleStyles reference type
        ...
        let module = if let Some(client_transition) = self.await?.client_transition {
            ...
            module_asset_context.process(source, new_reference_type)
        };
