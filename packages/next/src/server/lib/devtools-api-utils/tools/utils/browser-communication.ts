/**
 * Shared utilities for devtools API handlers that communicate with the browser.
 * This module provides a common infrastructure for request-response
 * communication between devtools API endpoints and browser sessions via HMR.
 */
...
export function createBrowserRequest<T>(
  ...
  const requestId = `devtools-${messageType}-${nanoid()}`
...
export function handleBrowserPageResponse<T>(
  ...
  if (!url) {
    throw new Error(
      'URL is required in devtools browser response. This is a bug in Next.js.'
    )
  }
