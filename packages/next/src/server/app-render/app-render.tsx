      ...
      if (needsRuntimePrefetchValidation(rscPayload)) {
        ...
        const reactServerStream = await workUnitAsyncStorage.run(
          ...
          () => {
            ...
            chunksPromise = collectStageChunksFromDynamicRender(
              teedStream,
              getStage
            )
            return stream
...
async function spawnDynamicValidationInDev(
  ...
  const getPayload = getRSCPayload.bind(null, tree, ctx, isNotFound)
  ...
  // Note: in the future, we might do something this in the dev render itself instead.
