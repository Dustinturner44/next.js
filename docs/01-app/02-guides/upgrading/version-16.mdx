...
However, in some cases, this might not be possible. In Webpack the `resolve.fallback` option was typically used to **silence** the error. Turbopack offers a similar option, using `turbopack.resolveAlias`. In this case, create an `empty.ts` file with no contents, and tell Turbopack to load it when `fs` is requested for the browser.
