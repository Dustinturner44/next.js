...
```
Rewrites are applied to both server-side and client-side routing. In the example above, a <Link href="/about"> will trigger a rewrite to `/`, just as if the user entered `/about` directly in the browser.
