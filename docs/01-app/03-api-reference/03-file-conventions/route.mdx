...
When reading files in a Route Handler (including from `public`), resolve the file path from the project root with `process.cwd()` and `path.join`. Avoid relative paths. Bundling can change source file locations and break relative paths.
