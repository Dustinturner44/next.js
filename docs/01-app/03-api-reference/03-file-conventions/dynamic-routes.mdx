...
export default async function Page({ params }: PageProps<'/blog/[slug]'>) {
  ...
  if (slug.startsWith('private-')) {
    ...
    return <PrivatePost slug={slug} token={token} />
  }
...
For runtime params not returned by `generateStaticParams`, validation occurs during the first request. In the example above, requests for slugs starting with `private-` will fail because `cookies()` is accessed without Suspense. Other runtime params that don't hit the conditional branch will render successfully and be saved to disk for subsequent requests.

If your application follows conditional paths like this, make sure to wrap runtime API access in Suspense boundaries:
