...
Remote caching provides the most value when content is deferred to request time (outside the static shell), for example after calling [`await connection()`](/docs/app/api-reference/functions/connection). In this context:
...
| **Latency impact**                      | None                                     | Network roundtrip on cache lookup  | None                   |
