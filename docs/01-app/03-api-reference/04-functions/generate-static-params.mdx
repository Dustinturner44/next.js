...
> - **With Cache Components**, returning an empty array is **not allowed** and will cause a [build error](/docs/messages/empty-generate-static-params). You must return at least one param for build-time validation.
