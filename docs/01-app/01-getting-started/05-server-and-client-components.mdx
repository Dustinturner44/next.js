...
### Reading files in Server Components

When a Server Component needs to read a local file, avoid relative paths like `./file.json` which depend on the current working directory. Instead, resolve paths relative to your project root using `process.cwd()` with `path.join`.

```ts filename="app/example/page.tsx" switcher
...
export default async function Page() {
  ...
  return <pre>{JSON.stringify(posts[0], null, 2)}</pre>
}
```

```js filename="app/example/page.js" switcher
import { readFile } from 'node:fs/promises'
import { join } from 'node:path'
